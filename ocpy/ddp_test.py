...
class DDPSolver(SolverBase):
    ...
    def init_solver(self):
        ...
        self._solve(
            *self._df, *self._dl, 
            ...
            alphas=self._alphas,
            stop_tol=self._stop_tol, max_iters=10, is_ddp=self._is_ddp
        )
    ...
    def solve(
        ...
        xs, us, ts, Js, is_success = self._solve(
            ...
            alphas, stop_tol, max_iters, self._is_ddp
        )
        # computational time
    ...
    def _solve(
            f, fx, fu, fxx, fux, fuu, l, lx, lu, lxx, lux, luu, lf, lfx, lfxx,
            t0, x0, T, N, us_guess, gamma_init, rho_gamma, gamma_min, gamma_max,
            alphas, stop_tol, max_iters, is_ddp
        ):
        """ DDP algorithm.
        """
        dt = T / N
...
def backward_pass(fx, fu, fxx, fux, fuu, 
                    ...
                    gamma: float=1e-6, is_ddp: bool=True):
